...
// Cargos de apoio nas colunas 7-15 (atual), 16-24 (ideal), 25-33 (saldo)
const CARGOS = ["ASG","AUX ADM","AUX EDUCAÇÃO","COORD. PÁTIO","LACTARISTA","LAVADEIRA","MERENDEIRO","PORTEIRO","ANALISTA ADM"]
const CARGOS_FULL = ["ASG","AUX ADM","AUX DE EDUCAÇÃO","COORDENADOR DE PÁTIO","LACTARISTA","LAVADEIRA","MERENDEIRA(O)","PORTEIRO","ANALISTA ADM"]

function processarIdeal(records) {
  const escolas = []
  // linha 0 = cabeçalho grupo, linha 1 = cabeçalho colunas
  ...
  const porTGS = {}
  escolas.forEach(e => {
    if (!porTGS[e.tgs]) porTGS[e.tgs] = { tgs: `TGS ${e.tgs}`, escolas: 0, faltando: 0, excedente: 0, atual: 0 }
...
// Dados fictícios para outros setores da Rede Física (T.I, Coord. Predial, Suporte)
...
export default function RedeFisicaPage() {
  ...
  if (carregando) return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#f0f4ff", fontFamily: "'Segoe UI', sans-serif" }}>
      <div style={{ textAlign: "center", color: COR }}>
        ...
        <div style={{ fontSize: 16, fontWeight: 600 }}>Carregando dados das escolas...</div>
  ...
  const kpis = [
    { label: "Escolas Monitoradas", valor: totalEscolas, icon: "🏫", variacao: "EM, CMEI, ETI e CEI" },
        ...
        {abaAtiva === "quadro" && (
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1.3fr 1fr", gap: 20, marginBottom: 24 }}>
              ...
              {/* Pizza por tipo + funcionários por função */}
              <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                <div style={{ background: "#fff", borderRadius: 16, padding: 24, boxShadow: `0 2px 12px ${COR}11` }}>
                  <div style={{ fontWeight: 700, fontSize: 14, color: COR, marginBottom: 2 }}>Escolas por Tipo</div>
            ...
            {/* Funcionários reais por função (Tab8) */}
            {funcoes && (
            ...
            {/* Top 10 mais defasadas */}
            <div style={{ background: "#fff", borderRadius: 16, padding: 24, boxShadow: `0 2px 12px ${COR}11`, marginBottom: 24 }}>
              <div style={{ fontWeight: 700, fontSize: 14, color: COR, marginBottom: 2 }}>🚨 Top 10 Escolas Mais Defasadas</div>
              <div style={{ fontSize: 11, color: "#94a3b8", marginBottom: 16 }}>Escolas com maior número de vagas abaixo do ideal</div>
              <ResponsiveContainer width="100%" height={260}>
            ...
            <div style={{ background: "#fff", borderRadius: 16, padding: 24, boxShadow: `0 2px 12px ${COR}11` }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16, flexWrap: "wrap", gap: 12 }}>
                <div>
                  <div style={{ fontWeight: 700, fontSize: 14, color: COR }}>Quadro Detalhado por Escola</div>
                ...
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
                  <input
                    ...
                    style={{ padding: "6px 12px", borderRadius: 20, border: `1px solid ${COR_BORDA}`, fontSize: 12, outline: "none", width: 180 }}
                  />
                  {tipos.map(t => (
                    <button key={t} onClick={() => setFiltroTipo(t)} style={{
                      padding: "5px 12px", borderRadius: 20, border: "none", cursor: "pointer",
                      fontSize: 11, fontWeight: 600,
                      background: filtroTipo === t ? (COR_TIPO[t] || COR) : COR_CLARA,
                      color: filtroTipo === t ? "#fff" : COR, transition: "all 0.2s",
                    }}>{t}</button>
                  ))}
              ...
              <div style={{ fontSize: 11, color: "#64748b", marginBottom: 12 }}>
                Exibindo <b>{escolasFiltradas.length}</b> de {totalEscolas} escolas
              </div>
              <div style={{ overflowX: "auto" }}>
                <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 800 }}>
                  <thead>
                    <tr style={{ borderBottom: `2px solid ${COR_CLARA}` }}>
                      {["TGS","TIPO","ESCOLA","ALUNOS","PORTE","ATUAL","IDEAL","FALTANDO","EXCEDENTE"].map(h => (
                ...
                {escolasFiltradas.length > 50 && (
                  <div style={{ textAlign: "center", padding: 16, fontSize: 12, color: "#94a3b8" }}>
                    Mostrando 50 de {escolasFiltradas.length} escolas. Use os filtros para refinar.
                  </div>
                )}
              </div>
            </div>
          ...
          <>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 24 }}>
              <div style={{ background: "#fff", borderRadius: 16, padding: 24, boxShadow: `0 2px 12px ${COR}11` }}>
                ...
                <div style={{ fontSize: 11, color: "#94a3b8", marginBottom: 16 }}>Zonas com maior defasagem no quadro de apoio</div>
                <ResponsiveContainer width="100%" height={260}>
                  <BarChart data={dadosTGS} barCategoryGap="25%">
                    ...
                    <Tooltip content={<TooltipCustom />} />
                    <Legend iconType="circle" iconSize={10} wrapperStyle={{ fontSize: 11 }} />
                    <Bar dataKey="faltando"  name="Faltando"  fill="#ef4444" radius={[4,4,0,0]} animationDuration={800} />
                    <Bar dataKey="excedente" name="Excedente" fill="#22c55e" radius={[4,4,0,0]} animationDuration={1000} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
              ...
              <div style={{ background: "#fff", borderRadius: 16, padding: 24, boxShadow: `0 2px 12px ${COR}11` }}>
                ...
                <div style={{ fontSize: 11, color: "#94a3b8", marginBottom: 16 }}>Total de quadro de apoio por zona</div>
                <ResponsiveContainer width="100%" height={260}>
                  <BarChart data={dadosTGS} barCategoryGap="30%">
                    ...
                    <Bar dataKey="atual" name="Funcionários ativos" radius={[4,4,0,0]} animationDuration={800}>
                      {dadosTGS.map((_, i) => <Cell key={i} fill={COR_TGS[i % COR_TGS.length]} />)}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 16 }}>
              {dadosTGS.map((tgs, i) => {
                const pctOk = Math.round(((tgs.atual - tgs.faltando) / tgs.atual) * 100)
                return (
                  <div key={tgs.tgs} style={{ background: "#fff", borderRadius: 14, padding: 20, boxShadow: `0 2px 12px ${COR}11`, borderTop: `4px solid ${COR_TGS[i]}` }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                      <div style={{ fontWeight: 800, fontSize: 16, color: COR_TGS[i] }}>{tgs.tgs}</div>
                      <span style={{ fontSize: 11, color: "#64748b" }}>{tgs.escolas} escolas</span>
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 12 }}>
                      <div style={{ textAlign: "center" }}>
                        <div style={{ fontSize: 18, fontWeight: 800, color: COR_TGS[i] }}>{tgs.atual}</div>
                        <div style={{ fontSize: 9, color: "#94a3b8" }}>Ativos</div>
                      </div>
                      <div style={{ textAlign: "center" }}>
                        <div style={{ fontSize: 18, fontWeight: 800, color: "#ef4444" }}>-{tgs.faltando}</div>
                        <div style={{ fontSize: 9, color: "#94a3b8" }}>Faltando</div>
                      </div>
                      <div style={{ textAlign: "center" }}>
                        <div style={{ fontSize: 18, fontWeight: 800, color: "#22c55e" }}>+{tgs.excedente}</div>
                        <div style={{ fontSize: 9, color: "#94a3b8" }}>Excedente</div>
                      </div>
                    </div>
                  </div>
                )
        ...
        <div style={{ background: "#fff", borderRadius: 16, padding: 24, boxShadow: `0 2px 12px ${COR}11`, marginTop: 24, borderLeft: `4px solid #94a3b8` }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, fontSize: 12, color: "#64748b", lineHeight: 1.7 }}>
            <div>
              ...
              Escolas que estão com <b>menos funcionários do que o ideal</b> para aquela função. Por exemplo, se o ideal é 4 merendeiros mas só há 2, o saldo é -2. Indica necessidade de contratação.
            </div>
            <div>
              <div style={{ fontWeight: 600, color: "#334155", marginBottom: 4 }}>🟢 Excedente (verde)</div>
              Escolas que estão com <b>mais funcionários do que o ideal</b>. Pode indicar que o quadro ideal da escola ainda não foi atualizado na planilha, ou que houve remanejamento interno.
            </div>
            <div>
              <div style={{ fontWeight: 600, color: "#334155", marginBottom: 4 }}>⚠️ Sobre os dados</div>
              Os valores exibidos são lidos diretamente da planilha do Google Sheets. Escolas com Nº Ideal = 0 podem indicar que o quadro ideal ainda não foi preenchido para aquela unidade.
            </div>
            <div>
              <div style={{ fontWeight: 600, color: "#334155", marginBottom: 4 }}>🗺️ O que é TGS?</div>
              TGS (Território de Gestão Setorial) é a divisão geográfica da cidade em zonas escolares. Caruaru está dividida em 9 TGS, cada uma agrupando escolas de uma região da cidade.
            </div>
          </div>
        </div>
