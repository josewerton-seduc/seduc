...
// Novos CMEIs para 2026 — informação fornecida pela gerente de organização escolar
const NOVOS_CMEIS_2026 = [
  "Rendeiras",
  "Sítio Cipó",
  "São João da Escócia",
  "Rafael",
...
function processarDados(csvEspera, csvMatric) {
  ...
  espera.forEach(r => {
    const t = r[23]?.trim()
    if (t) porTurma[t] = (porTurma[t]||0) + 1
  })
  ...
  const bairrosEspera = {}
  const bairrosMatric = {}
  espera.forEach(r => { const b = r[14]?.trim().toUpperCase(); if(b) bairrosEspera[b]=(bairrosEspera[b]||0)+1 })
  matric.forEach(r => { const b = r[8]?.trim().toUpperCase(); if(b) bairrosMatric[b]=(bairrosMatric[b]||0)+1 })

  // Top bairros na espera
  const topBairrosEspera = Object.entries(bairrosEspera)
    .map(([bairro, total]) => ({ bairro: bairro.slice(0,22), total }))
    .sort((a,b) => b.total - a.total).slice(0,12)

  // Pressão por bairro (espera/matriculados)
  const pressaoBairros = Object.entries(bairrosEspera)
    .filter(([b]) => b !== "...")
    .map(([b, e]) => {
      const m = bairrosMatric[b]||0
      const pressao = m === 0 ? (e > 0 ? 100 : 0) : Math.round(e/(m+e)*100)
      return { bairro: b.slice(0,22), espera: e, matriculados: m, pressao }
    })
    .filter(b => b.espera >= 10)
    .sort((a,b) => b.pressao - a.pressao)
    .slice(0, 12)

  // ── CMEI: fila + BF + PCD + faixa etária ────────────────
  const cmeiDados = {}
  espera.forEach(r => {
    const cmei = r[24]?.trim()
    if (!cmei) return
    if (!cmeiDados[cmei]) cmeiDados[cmei] = { total:0, bf:0, pcd:0, inf1:0, inf2:0, inf3:0, pts:[] }
    ...
    try { cmeiDados[cmei].pts.push(parseInt(r[28])) } catch {}
  })

  const escMatric = {}
  matric.forEach(r => {
    const n = normNome(r[2]?.trim()||"")
    escMatric[n] = (escMatric[n]||0)+1
  })

  // Top 12 CMEIs com mais fila — dados completos
  const topCmeiPrioridade = Object.entries(cmeiDados)
    .map(([cmei, d]) => {
      ...
      const ptsMed = d.pts.length > 0 ? (d.pts.reduce((a,b)=>a+b,0)/d.pts.length).toFixed(1) : 0
      return { cmei: nomeSimples.slice(0,35), total: d.total, bf: d.bf, pcd: d.pcd,
               inf1: d.inf1, inf2: d.inf2, inf3: d.inf3, capacidade: cap, pctBF, ptsMed: parseFloat(ptsMed) }
    })
    .sort((a,b) => b.total - a.total)
    .slice(0,12)

  // ── Modalidades matriculados ─────────────────────────────
  const porModalidade = {}
  matric.forEach(r => {
    const m = r[4]?.trim()||""
  ...
  return {
    totalEspera, totalMatric, comBF, pcdTotal, usaTransporte,
    turmasData, rendaData, topBairrosEspera, pressaoBairros,
    topCmeiPrioridade, topEscolas, porModalidade, espera,
    bairrosEspera, bairrosMatric,
  }
}
...
function Termometro({ pressao, label }) {
  const cor = COR_PRESSAO(pressao)
  const nivel = pressao >= 80 ? "🔴 CRÍTICO" : pressao >= 50 ? "🟠 ALTO" : pressao >= 25 ? "🟡 MÉDIO" : "🟢 OK"
  return (
    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:4 }}>
      <div style={{ fontSize:10, color:"#64748b", textAlign:"center", maxWidth:80 }}>{label}</div>
      <div style={{ width:12, height:60, background:"#e5e7eb", borderRadius:6, overflow:"hidden", position:"relative" }}>
        <div style={{ position:"absolute", bottom:0, width:"100%", height:`${pressao}%`, background:cor, borderRadius:6, transition:"height 0.6s ease" }}/>
      </div>
      <div style={{ fontSize:10, fontWeight:700, color:cor }}>{pressao}%</div>
      <div style={{ fontSize:9, color:cor }}>{nivel}</div>
    </div>
  )
}
...
export default function GGOrganizacaoPage() {
  ...
  const { totalEspera, totalMatric, comBF, pcdTotal, usaTransporte,
    turmasData, rendaData, topBairrosEspera, pressaoBairros,
    topCmeiPrioridade, topEscolas, porModalidade, espera } = dados

  // Aplica filtro interativo
  ...
  const pizzaModal = Object.entries(porModalidade)
    .map(([name, value], i) => ({ name, value, fill: ["#2d6a4f","#1d7fc4","#7c3371"][i] }))
    ...
    <div style={{ minHeight:"100vh", background:"#f4faf6", fontFamily:"'Segoe UI', sans-serif", color:"#1a3a2a" }}>
      ...
      <main style={{ padding:"92px 32px 52px" }}>
        ...
        <div style={{ background:"#fffbeb", border:"1.5px solid #fcd34d", borderRadius:10, padding:"10px 18px", marginBottom:20, fontSize:12, color:"#92400e" }}>
          ℹ️ <b>Escopo dos dados:</b> Este painel exibe exclusivamente dados de <b>CMEI/CEI</b>. Informações de Escolas Municipais (EM) e Escolas de Tempo Integral (ETI) não constam nas planilhas disponíveis.
        </div>
        ...
        {abaAtiva === "espera" && (
          <>
            {/* KPIs — removido "Bairros Sem CMEI" (dado impreciso) */}
            <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:16, marginBottom:24 }}>
              {[
                ...
                { label:"Com Bolsa Família",  valor:comBF.toLocaleString("pt-BR"),        icon:"💚", variacao:`${Math.round(comBF/totalEspera*100)}% das crianças na fila`, cor:"#15803d" },
                { label:"PCD na Fila",        valor:pcdTotal.toLocaleString("pt-BR"),     icon:"♿", variacao:"com necessidades especiais",  cor:"#7c3371" },
              ].map(k => (
                ...
                <ResponsiveContainer width="100%" height={200}>
                  <BarChart data={topBairrosFiltrado} layout="vertical" barCategoryGap="15%"
                    ...
                    <Bar dataKey="total" name="Na espera" radius={[0,4,4,0]} cursor="pointer" animationDuration={800}>
                      {topBairrosFiltrado.map((e) => <Cell key={e.bairro} fill={filtroAtivo?.valor === e.bairro ? "#0369a1" : COR}/>)}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              </div>
            ...
            {/* Renda — reduzida e ao lado dos novos CMEIs */}
            <div style={{ display:"grid", gridTemplateColumns:"1.6fr 1fr", gap:20 }}>
              <div style={{ background:"#fff", borderRadius:16, padding:24, boxShadow:`0 2px 12px ${COR}11` }}>
                <div style={{ fontWeight:700, fontSize:14, color:COR, marginBottom:2 }}>Perfil de Renda das Famílias na Fila</div>
                <div style={{ fontSize:11, color:"#94a3b8", marginBottom:4 }}>
                  {Math.round((rendaData.filter(r => r.faixa === "Até R$750" || r.faixa === "Sem renda").reduce((s,r)=>s+r.total,0)/totalEspera)*100)}% das famílias têm renda de até R$750 — maioria em situação de vulnerabilidade
                </div>
              ...
              {/* Novos CMEIs 2026 */}
              <div style={{ background:"#fff", borderRadius:16, padding:24, boxShadow:`0 2px 12px ${COR}11` }}>
                <div style={{ fontWeight:700, fontSize:14, color:COR, marginBottom:2 }}>🏗️ Novos CMEIs para 2026</div>
                <div style={{ fontSize:11, color:"#94a3b8", marginBottom:16 }}>Informação fornecida pela gerência de organização escolar</div>
                <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                  {NOVOS_CMEIS_2026.map((nome, i) => (
                    <div key={i} style={{ display:"flex", alignItems:"center", gap:10, background:COR_CLARA, borderRadius:10, padding:"10px 14px" }}>
                      <span style={{ fontSize:16 }}>📍</span>
                      <span style={{ fontSize:13, fontWeight:600, color:"#1a3a2a" }}>{nome}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <>
            ...
            <div style={{ background:"#fef2f2", border:"1.5px solid #fca5a5", borderRadius:12, padding:"14px 20px", marginBottom:24, display:"flex", gap:12, alignItems:"flex-start" }}>
              ...
              <div style={{ fontSize:13, color:"#7f1d1d" }}>
                <b>Atenção gerente:</b> os dados abaixo identificam as situações mais críticas — CMEIs com maior concentração de crianças vulneráveis e pressão de demanda por bairro. Esses são os pontos que precisam de ação prioritária.
              </div>
            </div>

            {/* Pressão por bairro — termômetros */}
            <div style={{ background:"#fff", borderRadius:16, padding:24, boxShadow:`0 2px 12px ${COR}11`, marginBottom:24 }}>
              ...
              <div style={{ fontSize:11, color:"#94a3b8", marginBottom:16 }}>
                % de crianças na espera em relação ao total (espera + matriculados no mesmo bairro) — quanto mais alto, mais pressão por vagas
              </div>
              <div style={{ display:"flex", gap:20, flexWrap:"wrap", justifyContent:"center" }}>
                {pressaoBairros.map(b => <Termometro key={b.bairro} pressao={b.pressao} label={b.bairro}/>)}
              </div>
            ...
            {/* Tabela de prioridade por CMEI */}
            <div style={{ background:"#fff", borderRadius:16, padding:24, boxShadow:`0 2px 12px ${COR}11` }}>
              <div style={{ fontWeight:700, fontSize:14, color:COR, marginBottom:2 }}>CMEIs: Fila, Vulnerabilidade e Faixa Etária</div>
              <div style={{ fontSize:11, color:"#94a3b8", marginBottom:16 }}>
                CMEIs com mais crianças vulneráveis (Bolsa Família + PCD) na fila de espera — base para priorização de vagas e expansão
              </div>
              <div style={{ overflowX:"auto" }}>
                <table style={{ width:"100%", borderCollapse:"collapse", minWidth:700 }}>
                  <thead>
                    <tr style={{ borderBottom:`2px solid ${COR_CLARA}` }}>
                      {["CMEI","FILA TOTAL","BOLSA FAMÍLIA","PCD","INFANTIL 1","INFANTIL 2","INFANTIL 3","MATRIC. ATUAL"].map(h => (
                  ...
                  <tbody>
                    {topCmeiPrioridade.map((d, i) => {
                      const pctBF = Math.round(d.bf/d.total*100)
                      const urgente = pctBF >= 70 || d.pcd > 0
                      return (
                        <tr key={i} style={{ borderBottom:`1px solid ${COR_CLARA}`, background: urgente ? "#fefce8" : i%2===0?"#fff":"#f8fbff" }}>
                          <td style={{ padding:"10px", fontSize:11, color:"#334155", fontWeight:500 }}>
                            {urgente && <span style={{ marginRight:4 }}>⚠️</span>}
                            {d.cmei}
                          </td>
                          ...
                          <td style={{ padding:"10px", textAlign:"center" }}>
                            ...
                            <div style={{ fontSize:9, color:"#94a3b8" }}>{pctBF}% da fila</div>
                          </td>
                          <td style={{ padding:"10px", textAlign:"center" }}>
                            {d.pcd > 0
                              ? <span style={{ background:"#fdf4ff", color:"#7c3371", borderRadius:20, padding:"2px 8px", fontSize:11, fontWeight:700 }}>{d.pcd}</span>
              ...
              <div style={{ marginTop:12, fontSize:11, color:"#64748b" }}>⚠️ Destacados em amarelo: CMEIs com mais de 70% da fila composta por famílias do Bolsa Família ou com crianças PCD.</div>
            </div>
          </>
        )}
          ...
          <>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:16, marginBottom:24 }}>
              {[
                ...
                { label:"Zona Rural",            valor:590, icon:"🌱", variacao:"da área rural", cor:"#7c3371" },
                { label:"Zona Urbana",           valor:"5.990", icon:"🏙️", variacao:"da área urbana", cor:"#c0521a" },
              ].map(k => (
                <div key={k.label} style={{ background:"#fff", borderRadius:14, padding:"18px 20px", boxShadow:`0 2px 12px ${k.cor}22`, borderLeft:`4px solid ${k.cor}`, display:"flex", alignItems:"center", gap:14 }}>
                  <span style={{ fontSize:28 }}>{k.icon}</span>
                  <div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1.5fr 1fr", gap:20 }}>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
                ...
                <div style={{ background:"#fff", borderRadius:16, padding:20, boxShadow:`0 2px 12px ${COR}11`, textAlign:"center" }}>
                  ...
                  <div style={{ fontSize:12, color:"#64748b" }}>{usaTransporte} crianças usam transporte público</div>
          ...
          <>
            <div style={{ background:"#f0f9ff", border:"1.5px solid #7dd3fc", borderRadius:12, padding:"14px 20px", marginBottom:24, display:"flex", gap:12, alignItems:"flex-start" }}>
              ...
              <div style={{ fontSize:13, color:"#0369a1" }}>
                Esta aba cruza os dados da fila com os matriculados. CMEIs com <b>0 matriculados</b> podem ser unidades ainda sem turmas formadas — a demanda na fila já aponta onde a expansão é mais urgente.
              </div>
            </div>

            <div style={{ background:"#fff", borderRadius:16, padding:24, boxShadow:`0 2px 12px ${COR}11`, marginBottom:24 }}>
              <div style={{ fontWeight:700, fontSize:14, color:COR, marginBottom:2 }}>CMEIs: Fila de Espera vs. Matriculados Atuais</div>
              <div style={{ fontSize:11, color:"#94a3b8", marginBottom:16 }}>Barra laranja maior que a verde = demanda supera capacidade atual</div>
            ...
            <div style={{ background:"#fff", borderRadius:16, padding:24, boxShadow:`0 2px 12px ${COR}11` }}>
              <div style={{ fontWeight:700, fontSize:14, color:COR, marginBottom:2 }}>Bairros: Fila vs. Matriculados</div>
              <div style={{ fontSize:11, color:"#94a3b8", marginBottom:16 }}>Onde há mais pressão por novas vagas na rede</div>
              <ResponsiveContainer width="100%" height={280}>
